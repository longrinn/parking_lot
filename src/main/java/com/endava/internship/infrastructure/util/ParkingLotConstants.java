package com.endava.internship.infrastructure.util;

public class ParkingLotConstants {

    public static final String USER_NOT_FOUND_ERROR_MESSAGE = "User with email=[%s] was not found.";
    public static final String USER_EMAIL_NOT_FOUND_ERROR_MESSAGE = "User with Email=[%s] was not found.";
    public static final String ROLE_NOT_FOUND_ERROR_MESSAGE = "Role=[%s] was not found.";
    public static final String CREDENTIALS_NOT_FOUND_ERROR_MESSAGE = "Credentials for User email=[%s] were not found.";
    public static final String PARKING_LOT_NAME_NOT_FOUND_ERROR_MESSAGE = "Parking lot with Name=[%s] was not found.";
    public static final String PARKING_LOT_ID_NOT_FOUND_ERROR_MESSAGE = "Parking lot with Id = [%d] was not found.";
    public static final String PARKING_SPOT_NOT_FOUND_ERROR_MESSAGE = "Parking spot with ID=[%s] was not found.";
    public static final String ENTITIES_ALREADY_LINKED = "Entities are already linked";
    public static final String ENTITIES_NOT_LINKED = "Entities aren't linked";
    public static final String PARKING_LOT_ID_NOT_FOUND_MESSAGE = "Parking lot with ID=[%s] was not found";
    public static final String SPOT_NOT_FOUND = "Parking spot was not found";
    public static final String SPOT_UNAVAILABLE = "Parking spot is unavailable now";
}